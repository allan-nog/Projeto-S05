...
function openMenu() { menuAba.style.display = 'block'; }
function closeMenu() { menuAba.style.display = 'none'; }

menuAba?.addEventListener('click', (e) => {
  const btn = e.target.closest('.menu_bot');
  if (!btn) return;
  const t = btn.dataset.theme;
  closeMenu();
  if (t === 'inatel') temaInatel();
  else if (t === 'limao') temaLim();
  else if (t === 'dark') temaDark();
});

menuButton?.addEventListener('click', () => {
  const visible = menuAba.style.display === 'block';
